...
class Attend(nn.Module):
    ...
    def flash_attn(
        ...
        if exists(mask) and causal:
            ...
            causal = False
        # if exists(mask) and mask.all()==False:
            # protect against an entire row being masked out
            # mask.masked_fill_(~mask.any(-1, keepdim=True), True)
            # row_is_entirely_masked = ~mask.any(dim = -1, keepdim=True)
            # mask = mask | row_is_entirely_masked
        ...
        if exists(row_is_entirely_masked):
            ...
            # out = out.masked_fill(row_is_entirely_masked, 0.)
    ...
    def forward(
        ...
        if self.flash:
            ...
            return self.flash_attn(q, k, v, causal, mask = mask, attn_bias = attn_bias)
