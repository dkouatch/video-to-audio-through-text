...
class DataCollatorForSeq2Seq2:
    ...
    def load_video_feature(self, filename):
        if filename.endswith(".npy"):
            video_feat = np.load(filename)
            video_feat = torch.FloatTensor(video_feat).unsqueeze(0)
        else:
            with h5py.File(filename, "r") as hf:
                video_feat = hf["clip"][:]
            video_feat = torch.FloatTensor(video_feat)
            video_feat = torch.repeat_interleave(video_feat, 5, dim=0).unsqueeze(0)
        return video_feat
    ...
    def __call__(self, features, return_tensors=None):
        ...
        # add video, temperally save to another variable as it does not need pad
        videos = []
        for i, feature in enumerate(new_features):
            # if "video_path" not in feature:
            #     print(feature)
            videos.append(self.load_video_feature(feature.pop("video_path")))
        videos = torch.cat(videos, dim=0)

        # remove unnecessary column to avoid bugs in the following pad forward
        for key in ['task', 'output', 'instruction']:
            for feature in new_features:
        ...
        # video needs to add back
        # new_features["video_input"] = videos.half()
        new_features["video_input"] = videos

        # adjust the attention mask
        ori_att_mask = new_features["attention_mask"]
        video_att_mask = torch.ones([ori_att_mask.shape[0], 10]) # batch size [2, 10]
        new_features["attention_mask"] = torch.concat([video_att_mask, ori_att_mask], dim=1)
