        ...
        if not isinstance(self.model, PreTrainedModel):
            if isinstance(unwrap_model(self.model), PreTrainedModel):
                ...
                for name, param in state_dict.items():
                    # Uncomment this line if just finetune using lora
...
def create_fz_optimizer(self):
    ...
    if self.optimizer is None:
        ...
        optimizer_grouped_parameters = [
            {"params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'video' in n)], "weight_decay": self.args.weight_decay,},
            {"params": [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad and 'video' in n)], "weight_decay": 0, },
            {"params": [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'video' not in n)], "weight_decay": 0, 'lr': 0},
        ]

        print('Total video parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'video' in n)]) / 1e6))
        print('Total video parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n not in decay_parameters and p.requires_grad and 'video' in n)]) / 1e6))
        print('Total llm parameter number is : {:.3f} million'.format(sum(p.numel() for p in [p for n, p in opt_model.named_parameters() if (n in decay_parameters and p.requires_grad and 'video' not in n)]) / 1e6))
