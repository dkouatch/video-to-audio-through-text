    ...
    def sample(
        ...
        **model_kwargs,
        ...
        if video_input != None:
            ...
            video_att_mask = torch.ones((ori_att_mask.shape[0], int(video_input.shape[1]/5)), dtype=torch.bool, device=ori_att_mask.device)
            model_kwargs['attention_mask'] = torch.concat([ori_att_mask, video_att_mask], dim=1)
        else:
            pass
        ...
        while True:
            ...
            if loop_idx != 0:
                video_input = None
            model_inputs = self.prepare_inputs_for_generation(input_ids, video_input, **model_kwargs)
            loop_idx += 1
            outputs = self(
                **model_inputs,
                return_dict=True,
