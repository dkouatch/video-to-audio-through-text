...
if __name__ == "__main__":
    ...
    if not is_train:
        audio_path_base = "/pscratch/sd/x/xiuliu/vggsound_audio_wav/test/"
    else:
        audio_path_base = "/pscratch/sd/x/xiuliu/vggsound_audio_wav/train/"
    all_files = os.listdir(audio_path_base)
    files_load = all_files
    if is_train and slice < 7:
        files_load = all_files[int(slice * len(all_files) // 8): int((slice + 1) * len(all_files) // 8)]
    elif is_train and slice == 7:
    ...
    for audio_name in tqdm(files_load):
        audio_path = os.path.join(audio_path_base, audio_name)
        ...
        with open("/pscratch/sd/x/xiuliu/vggsound_audio_caption/{}/{}.txt".format(save_folder, audio_name[:-4]), "w+") as fout:
            fout.write(pred)
