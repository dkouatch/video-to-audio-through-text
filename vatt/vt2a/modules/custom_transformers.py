...
class AdaLNModulation(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # B, D -> B, D/2
        scale, shift = self.linear(context).chunk(2, dim=-1)
...
class ContinuousTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert isinstance(attn_layers, AttentionLayersKS) or isinstance(attn_layers, EncoderAttentionLayersKS), 'attention layers must be one of Encoder or Decoder'
...
class AttentionLayersKS(nn.Module):
    def __init__(
        self,
        dim,
        depth,
    ...
    def forward(
        ...
        for ind, (layer_type, (norm, block, residual_fn), layer_dropout) in enumerate(zip(self.layer_types, self.layers, self.layer_dropouts)):
            ...
            if layer_type == 'a':
                out, inter = block(x, causal=causal, mask = mask, context_mask = self_attn_context_mask, attn_mask = attn_mask, rel_pos = self.rel_pos, rotary_pos_emb = rotary_pos_emb, prev_attn = prev_attn, mem = layer_mem)
            elif layer_type == 'c':
                out, inter = block(x, causal=causal, context = context, mask = mask, context_mask = context_mask, prev_attn = prev_cross_attn)
            elif layer_type == 'f':
                out = block(x)
            elif layer_type == 'm':
...
class DecoderKS(AttentionLayersKS):
...
class MultiwayNetwork(nn.Module):
    ...
    def forward(self, x, split_position, av_order):
        if split_position == -1:
            return self.A(x)
        if split_position == 0:
            return self.B(x)
        ...
        x1, x2 = torch.split(
            x,
            [split_position, x.size(self.dim) - split_position],
            dim=self.dim,
        )
        # x1, x2 = x[:self.split_position], x[self.split_position:]
        if av_order[0] == 'v':
            y1, y2 = self.A(x1), self.B(x2)
        else:
...
class EncoderAttentionLayersKS(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.multiway:
            # multi_norm = MultiwayNetwork(norm_class, dim)
            # norm_fn = multi_norm
            norm_fn = MultiwayWrapper(norm_class(dim))
            # norm_fn = partial(multi_norm, dim=1)
            # print(type(norm_fn))
        else:
            norm_fn = partial(norm_class, dim)
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                ...
                sub_layer = FeedForward(dim, **ff_kwargs)
                layer = MultiwayWrapper(sub_layer)
                # layer = partial(MultiwayNetwork, FeedForward, dim, **ff_kwargs)
            elif layer_type == 'm':
                # sub_layer = AdaLNModulation(dim)
                # layer = MultiwayWrapper(sub_layer)
                layer = AdaLNModulation(dim)
            else:
                raise Exception(f'invalid layer type {layer_type}')
    ...
    def forward(
        ...
        for ind, (layer_type, (norm, block, residual_fn), layer_dropout) in enumerate(zip(self.layer_types, self.layers, self.layer_dropouts)):
            ...
            if exists(pre_norm):
                if self.multiway:
                    x = pre_norm(x, split_pos, av_order)
                else:
                    x = pre_norm(x)
            ...
            if layer_type == 'a':
                ...
                out = block(x, split_pos, av_order)
            elif layer_type == 'm':
                if exists(split_pos):
                    out = block(x[:, split_pos:, :], context = context)
                    out = torch.cat([x[:, :split_pos, :], out], dim=1)
                else:
                    out = block(x, context=context)
        ...
        if self.resi_dual:
            ...
            if self.multiway:
                x = self.final_norm(x, split_pos, av_order)
            else:
                x = self.final_norm(x)
