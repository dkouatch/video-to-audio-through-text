...
AUDIOSET_PROMPT_UNBALANCED_PATH = "/pscratch/sd/x/xiuliu/ltu/data/audioset_unbalanced_train_v2a_instruction.json"
AUDIOSET_PROMPT_BALANCED_PATH = "/pscratch/sd/x/xiuliu/ltu/data/audioset_balanced_train_v2a_instruction.json"
AUDIOSET_PROMPT_EVAL_PATH = "/pscratch/sd/x/xiuliu/ltu/data/audioset_eval_v2a_instruction.json"
...
class MixDataset(Dataset):
    def __init__(self, split, stage=1, clip_interp=True, prompt_template_name="alpaca_short", base_model="/pscratch/sd/x/xiuliu/ltu/pretrained_mdls/vicuna/"):
        assert stage in [1, 2], "Stage must be either 1 or 2!"
        self.split = split
        ...
        if stage == 1:
            ...
            with open(AUDIOSET_PROMPT_UNBALANCED_PATH, "r") as fin:
                instructions_unb = json.load(fin)
            for item in instructions_unb:
                self.text_prompts[item["video_path"]] = item["output"] #os.path.basename(item["video_path"]).split(".")[0]
            with open(AUDIOSET_PROMPT_BALANCED_PATH, "r") as fin:
                instructions_b = json.load(fin)
            for item in instructions_b:
                self.text_prompts[item["video_path"]] = item["output"]
            with open(AUDIOSET_PROMPT_EVAL_PATH, "r") as fin:
                instructions_eval = json.load(fin)
            for item in instructions_eval:
                self.text_prompts[item["video_path"]] = item["output"]
    ...
    def __getitem__(self, idx):
        ...
        if data_n == 'as':
            ...
            clip_emb = torch.repeat_interleave(torch.from_numpy(clip_emb), 5, dim=0).float()
        else:
        ...
        no_help = False
        if self.stage == 1 or (self.stage == 2 and random.random() < 0.3) or (self.stage == 2 and fn not in self.text_prompts):
        ...
        # if self.stage == 2 and random.random() < 0.25:
        ...
        if self.split == "train":
            ...
            if aug_prob < 0.5 and no_help:
                ...
                if aug_data_n == 'as':
                    ...
                    aug_clip_emb = torch.repeat_interleave(torch.from_numpy(aug_clip_emb), 5, dim=0).float()
        ...
        # print("HHH", fn, data_n, clip_emb.shape)
    ...
    def tokenize(self, prompt, add_eos_token=True, max_length=108):
...
if __name__ == "__main__":
    ...
    dataset = MixDataset(split="train", stage=2)
    data_collator = VT2A_Collator(tokenizer_path="/pscratch/sd/x/xiuliu/ltu/pretrained_mdls/vicuna/")
    dataloader = DataLoader(dataset=dataset, batch_size=64, collate_fn=data_collator, num_workers=12, shuffle=False)
    print(len(dataset))
    t = time()
